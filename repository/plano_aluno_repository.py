...
def cadastrar_plano_aluno(id_plano: int, id_aluno: int, metodo_pagamento: str, data_hora: str):
    try:
...
def listar_plano_aluno(id_aluno: int) -> list | None:
    try:
        conexao = criar_conexao()
        cursor = conexao.cursor()
        cursor.execute("SELECT p.* FROM plano_aluno pa JOIN planos p ON p.id_plano = pa.id_plano WHERE id_aluno = %s ORDER BY id_plano ASC;", [id_aluno])
...
def deletar_plano_aluno(id_plano: int, id_aluno: int):
    try:
        conexao = criar_conexao()
        cursor = conexao.cursor()
        cursor.execute("DELETE FROM plano_aluno WHERE id_plano = %s AND id_aluno = %s;", [id_plano, id_aluno])
        nome_plano = listar_nome_plano(id_plano)
        conexao.commit()
